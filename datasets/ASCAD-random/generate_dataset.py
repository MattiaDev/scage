...
def desync_trace(trace, amount=100):
    trace_len = len(trace)
    trace_shifted = np.zeros(trace_len)
    shift = random.randint(-amount//2, amount//2)
    if shift > 0:
        trace_shifted[0:trace_len - shift] = trace[shift:trace_len]
        trace_shifted[trace_len - shift:trace_len] = trace[0:shift]
    else:
        trace_shifted[0:abs(shift)] = trace[trace_len - abs(shift):trace_len]
        trace_shifted[abs(shift):trace_len] = trace[0:trace_len - abs(shift)]
    return trace_shifted


def generate_nopoi(raw_trace_file, window, amount=0):
    n_profiling = 200000
    n_attack = 10000
    ...
    in_file = h5py.File(raw_trace_file, "r")
    ...
    for i, j in tqdm(enumerate(profiling_index)):
        if amount != 0:
            profiling_samples[i] = winres(desync_trace(traces[j], amount), window=window)
        else:
    ...
    for i, j in tqdm(enumerate(attack_index)):
        if i == 10000:
            break
        if amount != 0:
            attack_samples[i] = winres(desync_trace(traces[j], amount), window=window)
        else:
    ...
    if amount != 0:
        out_file = h5py.File(f'ascad_random_nopoi_window_{window}_desync{amount}.h5', 'w')
    else:
        out_file = h5py.File(f'ascad_random_nopoi_window_{window}.h5', 'w')
...
if __name__ == "__main__":
    ...
    #generate_nopoi(window=20, desync=True)
    #generate_nopoi(sys.argv[1], window=40, amount=50)
    generate_nopoi(sys.argv[1], window=int(sys.argv[2]), amount=int(sys.argv[3]))
