...
def generate_nopoi(raw_trace_file, window, amount):
    print(f'Processing {raw_trace_file} with window={window} and desync of {amount}')
    n_profiling = 50000
    n_attack = 10000
    ...
    for trace_index in tqdm(range(n_profiling + n_attack)):
        if amount != 0:
            profiling_samples[trace_index] = winres(desync_trace(traces[trace_index], amount), window=window)
        else:
    ...
    if amount != 0:
        out_file = h5py.File(f'ascad_fixed_nopoi_window_{window}_desync{amount}.h5', 'w')
    else:
        out_file = h5py.File(f'ascad_fixed_nopoi_window_{window}.h5', 'w')
