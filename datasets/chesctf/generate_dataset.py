...
dataset_folder_chesctf_opoi = '.'
dataset_folder_chesctf_nopoi = '.'
dataset_folder_chesctf_nopoi_desync = '.'
...
def load_trs_trace(filename, number_of_traces, number_of_samples, data_length, number_of_samples_resampled=None, window=20, desync=0):
    ...
    for i, trace in enumerate(trace_file[:number_of_traces]):
        ...
        if number_of_samples_resampled is not None:
            ...
            if desync != 0:
                ...
                shift = random.randint(-desync//2, desync//2)
                if shift > 0:
...
def generate_nopoi_desync(window, desync=100):
    n_traces_file = 10000
    n_profiling = 30000
    n_attack = 10000
    number_of_samples = int(150000 / window) * 2
    ...
    samples, plaintexts, ciphertexts, keys = load_trs_trace(f"{raw_trace_folder_chesctf}/PinataAcqTask2.1_10k_upload.trs", n_traces_file,
                                                            150000, 48,
                                                            number_of_samples_resampled=number_of_samples, window=window, desync=desync)
    profiling_samples[:n_traces_file] = samples
    profiling_plaintexts[:n_traces_file] = plaintexts
    profiling_ciphertexts[:n_traces_file] = ciphertexts
    profiling_keys[:n_traces_file] = keys
    ...
    attack_samples, attack_plaintexts, attack_ciphertexts, attack_keys = load_trs_trace(
        ...
        n_traces_file, 150000, 48,
        number_of_samples_resampled=number_of_samples,
        window=window, desync=desync)

    out_file = h5py.File(f'{dataset_folder_chesctf_nopoi_desync}/ches_ctf_nopoi_window_{window}_desync{desync}.h5', 'w')
...
if __name__ == "__main__":
    ...
    ##generate_nopoi(80)
    #generate_opoi()
    generate_nopoi_desync(40, 50)
    generate_nopoi_desync(40, 100)
    generate_nopoi_desync(40, 200)
