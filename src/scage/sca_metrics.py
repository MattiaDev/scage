...
def sca_metrics(model, x_data, n_ge, label_key_guess, correct_key):
    ...
    output_probabilities = np.log(model.predict(x_data, verbose=0) + 1e-36)
    ...
    if guessing_entropy[n_ge - 1] < 2:
        result_number_of_traces_ge_1 = int(n_ge - np.argmax(guessing_entropy[::-1] > 2))
    else:
        result_number_of_traces_ge_1 = int(n_ge)
