...
class ReadCHESCTF:
    ...
    def __init__(self, n_profiling, n_validation, n_attack_evo, n_attack, file_path="", target_byte=0, number_of_samples=2000, is_desync=False, reshape_to_cnn=True):
        self.n_profiling = n_profiling
        self.n_validation = n_validation
        self.n_attack_evo = n_attack_evo
        self.n_attack = n_attack
        self.target_byte = target_byte
        self.file_path = file_path
        self.ns = number_of_samples
        self.classes = 256
        self.is_desync = is_desync
        ...
        self.load_dataset(reshape_to_cnn)

    def load_dataset(self, reshape_to_cnn):
        ...
        profiling_plaintexts = profiling_plaintext[:-self.n_validation]
        profiling_keys = profiling_key[:-self.n_validation]
        validation_plaintexts = profiling_plaintext[-self.n_validation:]
        validation_keys = profiling_key[-self.n_validation:]
        attack_evo_plaintexts = attack_plaintext[:self.n_attack_evo]
        attack_evo_keys = attack_key[:self.n_attack_evo]
        attack_plaintexts = attack_plaintext[self.n_attack_evo:self.n_attack_evo + self.n_attack]
        attack_keys = attack_key[self.n_attack_evo:self.n_attack_evo + self.n_attack]

        x_profiling = profiling_samples[0:-self.n_validation, :self.ns]
        x_validation = profiling_samples[-self.n_validation:, :self.ns]
        x_attack_evo = attack_samples[0:self.n_attack_evo, :self.ns]
        x_attack = attack_samples[self.n_attack_evo:self.n_attack_evo + self.n_attack, :self.ns]

        if not self.is_desync:
            ...
            self.apply_z_score_norm(x_attack_evo, z_score_mean, z_score_std)
            self.apply_z_score_norm(x_attack, z_score_mean, z_score_std)
        else:
            x_profiling = self.apply_horizontal_norm(x_profiling)
            x_validation = self.apply_horizontal_norm(x_validation)
            x_attack_evo = self.apply_horizontal_norm(x_attack_evo)
            x_attack = self.apply_horizontal_norm(x_attack)

        self.x_profiling = x_profiling.astype('float32')
        self.x_validation = x_validation.astype('float32')
        self.x_attack_evo = x_attack_evo.astype('float32')
        ...
        if reshape_to_cnn:
            ...
            self.x_attack_evo = self.x_attack_evo.reshape((self.x_attack_evo.shape[0], self.x_attack_evo.shape[1], 1))
        ...
        self.attack_evo_labels = self.aes_labelize(attack_evo_plaintexts, attack_evo_keys)
        ...
        self.y_attack_evo = to_categorical(self.attack_evo_labels, num_classes=self.classes)
        ...
        self.labels_key_hypothesis_attack_evo = self.create_labels_key_guess(attack_evo_plaintexts)
    ...
    def apply_horizontal_norm(self, dataset):
        z_score_mean = np.mean(dataset, axis=1, keepdims=True)
        z_score_std = np.std(dataset, axis=1, keepdims=True)
        mean_v = np.repeat(z_score_mean, dataset.shape[1], axis=1)
        std_v = np.repeat(z_score_std, dataset.shape[1], axis=1)
        return (dataset - mean_v) / std_v
